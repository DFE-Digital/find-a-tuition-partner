        ...
        await strategy.ExecuteAsync(
            async () =>
            {
                ...
                foreach (var resourceName in assembly.GetManifestResourceNames())
                {
                    ...
                    {
                        _logger.LogError($"Tuition Partner resource file {originalFilename} not found");
                        continue;
                    }

                    _logger.LogInformation($"Attempting create Tuition Partner from resource file {originalFilename}");
                    ...
                    {
                        _logger.LogError(ex, $"Exception thrown when creating Tuition Partner file {originalFilename}");
                        continue;
                    }
                    ...
                    if (!results.IsValid)
                    {
                        _logger.LogError($"Tuition Partner name {tuitionPartner.Name} is not valid.{Environment.NewLine}{string.Join(Environment.NewLine, results.Errors)}");
                        continue;
                    }
                    ...
                    _logger.LogInformation($"Added Tuition Partner {tuitionPartner.Name} with id of {tuitionPartner.Id}");
                }
