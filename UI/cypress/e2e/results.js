...
import {kebabCase, KeyStageSubjects, removeExcessWhitespaces, removeNewLine} from "../support/utils";
...
Then("{string} is not shortlisted", (tpName) => {
    cy.get(`[id="shortlist-cb-${kebabCase(tpName)}"]`).should("not.be.checked");
});
...
Then("{string} is not shortlisted for {string} on its detail page", (tpName, laName) => {
    const tpCheckboxSelector = `[id="shortlist-tpInfo-cb-${kebabCase(tpName)}"]`;
    cy.getCookie(".FindATuitionPartner.Shortlist").should("be.null");
    cy.get('[data-testid="la-name"]').should("contain.text", `Tuition partner for ${laName}`);
    cy.get(tpCheckboxSelector).should("not.be.checked");
});
...
Then("they add {string} to the shortlist", (tpName) => {
    const tpCheckboxSelector = `[id="shortlist-cb-${kebabCase(tpName)}"]`;
    cy.get(tpCheckboxSelector).check();
    cy.get(tpCheckboxSelector).should("be.checked");
});

Then("{string} is now shortlisted {string} on its detail page", (tpName, laName) => {
    const seoUrl = kebabCase(tpName);
    cy.get(`[id="shortlist-tpInfo-cb-${seoUrl}"]`).should("be.checked");
    cy.getCookie(".FindATuitionPartner.Shortlist").should(
        "have.property",
        "value",
        seoUrl
    );
    cy.get('[data-testid="la-name"]').invoke("text")
        .then((text)=>{
           cy.wrap( removeExcessWhitespaces(removeNewLine(text))).should(
        "contain",
        `Shortlisted tuition partner for ${laName}`
    );
})});

Then("they remove {string} from the shortlist",(tpName)=>{
    const tpCheckboxSelector = `[id="shortlist-cb-${kebabCase(tpName)}"]`;
    cy.get(tpCheckboxSelector).uncheck();
    cy.get(tpCheckboxSelector).should("not.be.checked");
});

Then("{string} is longer shortlisted for {string} on its detail page",(tpName,laName)=>{
    const seoUrl = kebabCase(tpName);
    cy.getCookie(".FindATuitionPartner.Shortlist").should("have.property", "value", "");
    cy.get('[data-testid="la-name"]').should("contain.text", `Tuition partner for ${laName}`);
    cy.get(`[id="shortlist-tpInfo-cb-${seoUrl}"]`).should("not.be.checked");
});