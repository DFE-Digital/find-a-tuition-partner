...
Given(
    ...
    () => {
        cy.visit("https://localhost:7036/enquiry/respond/response-confirmation?SupportReferenceNumber=SJ4798&EnquirerMagicLink=FmeOs4T1Y9uTU0Q8rR3Qo9xDopWyHBBD5GKuukQHsUoevwBuM3ftXccCD22NBmdLITdbrgKl84xRd%2FOCFvcRReP0w3Nv%2BKVrsehSIboFCk4f%2FVYPMLNbPxTij7fnvEkED7mce6UnnPga%2FfEYNbLyMQ%3D%3D")
        cy.get('.govuk-panel__body > strong').invoke('text').then((text) => refrenceNumber = text);
...
When("they click all the view your tuition requirement links the text shows", () => {
    cy.get(':nth-child(14) > .govuk-details__summary > .govuk-details__summary-text').click()
    cy.get(':nth-child(19) > .govuk-details__summary > .govuk-details__summary-text').click()
    cy.get(':nth-child(24) > .govuk-details__summary > .govuk-details__summary-text').click()
})
    ...
    dataTable.hashes().forEach((row) => {
        ...
        switch (requirement) {
            case "Key stage and subjects:":
                cy.get('.govuk-grid-column-two-thirds-from-desktop > .govuk-list > :nth-child(1)')
                    .should("contain.text", 'Key stage 1: English and Maths')
                    .should('be.visible');
                cy.get('.govuk-grid-column-two-thirds-from-desktop > .govuk-list > :nth-child(2)')
                    .should("contain.text", 'Key stage 2: English and Maths')
                    .should('be.visible');
                cy.get(':nth-child(3) > .govuk-grid-row > .govuk-grid-column-two-thirds-from-desktop > :nth-child(6)')
