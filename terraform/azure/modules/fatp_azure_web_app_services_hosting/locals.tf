locals {
  ...
  should_create_fatp_web_kv_policy      = local.is_access_policies_objectIds_empty ? 1 : (contains(local.existing_kv_access_policies_objectIds, data.azurerm_linux_web_app.fatp_web_app[0].identity[0].principal_id) ? 0 : 1)
}
